...
stage_clf = joblib.load(r"model\correction\models\model_barbell_bieceps.pkl")
label_encoder = joblib.load(r"model\correction\models\label_encoder_barbell.pkl")
thresholds = json.load(open(r"model\correction\thresholds\thresholds_barbell.json"))
...
def smooth_phase(current_phase, new_phase, prev_landmarks):
    """Lisse la détection de phase pour éviter les fluctuations."""
    prev_landmarks.append(new_phase)
    if len(prev_landmarks) > PHASE_SMOOTHING_FRAMES:
        prev_landmarks.pop(0)
    # Retourne la phase majoritaire dans l'historique
    return max(set(prev_landmarks), key=prev_landmarks.count)
...
def correct_barbell_biceps(frame, results, prev_landmarks, counter, current_phase): 
    h, w = frame.shape[:2]
    ...
    if results.pose_landmarks:
        ...
        if min(vis_ls, vis_le, vis_lw, vis_rs, vis_re, vis_rw) >= VISIBILITY_THRESH:
            ...
            if prob >= PROBA_THRESH:
                phase = smooth_phase(current_phase, phase, prev_landmarks)
            ...
            if phase in ("haut", "bas"):
                ...
                for name, (val, (lo, hi)) in ratio_checks.items():
                    ...
                    if "Elbow" in name:
                        cv2.line(frame, Lel, Rel, color, 2)
                    elif "Wrist" in name:
                        cv2.line(frame, Lwr, Rwr, color, 2)

                # Dessiner la ligne entre les épaules en blanc (référence)
                cv2.line(frame, Lsh, Rsh, (255, 255, 255), 2)
    ...
    for i, text in enumerate(info_texts):
        ...
        cv2.putText(frame, text, (feedback_x, feedback_y_start + i * line_height), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1, cv2.LINE_AA)
